...
description: "Register remote MCP servers with MCPJungle over streamable HTTP, using either CLI flags or config files with bearer tokens and custom headers."
---

MCPJungle can proxy any MCP server that speaks the [Streamable HTTP transport](https://modelcontextprotocol.io/specification/2025-03-26/basic/transports#streamable-http).

Once registered, that server's tools become available through the MCPJungle gateway and can be exposed to clients through the main `/mcp` endpoint or through Tool Groups.
...
<CardGroup cols={2}>
  ...
  <Card title="Upstream authentication" icon="key" href="/governance/upstream-authentication">
    Understand bearer tokens and custom headers for upstream MCP servers.
  </Card>
