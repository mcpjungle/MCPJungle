...
Disabling a server disables all of its tools and prompts in one step:
...
| Context | Behavior |
|---|---|
| Main gateway (`/mcp`) | Disabled tools and prompts are not listed and cannot be called. |
...
- Prompts are currently **not** exposed through Tool Group endpoints.
