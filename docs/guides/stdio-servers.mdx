...
description: "Register STDIO-based MCP servers that run as local child processes, including filesystem, GitHub, and other servers launched with npx or uvx."
---
...
These servers run as child processes on the same machine as the MCPJungle server and communicate through standard input and output. Common examples include `filesystem`, `time`, and `github`.
...
<CardGroup cols={2}>
  ...
  <Card title="Register HTTP servers" icon="server" href="/guides/register-servers">
