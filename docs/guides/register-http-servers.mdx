...
If the upstream server advertises `tools.listChanged`, Mcpjungle also keeps a background listener connection open so tool additions and removals can be synced without restarting the gateway.
...
If the upstream later sends `notifications/tools/list_changed`, Mcpjungle automatically refreshes its cached tool list and downstream MCP clients connected to Mcpjungle can refresh in response to the propagated notification.
