...
description: "Create named subsets of tools with their own MCP endpoint so each client only sees the tools it needs instead of the full registry surface."
---

As you register more MCP servers, the full tool catalog can become too large for a single agent workflow. Tool Groups solve that by exposing a named subset of tools through a dedicated MCP endpoint.

Each client can then connect to the specific group endpoint instead of the main `/mcp` gateway.
