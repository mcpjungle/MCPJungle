...
<ParamField path="SQLITE_DB_PATH" type="string">
  Path to the SQLite database file to use when `DATABASE_URL` and `POSTGRES_HOST` are not set. The `--sqlite-db-path` CLI flag takes precedence over this variable.
  ...
  When this variable is set, MCPJungle uses that exact path instead of auto-detecting `mcpjungle.db` or the legacy `mcp.db`.
</ParamField>
