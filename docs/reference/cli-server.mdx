...
<ParamField body="--ui" type="boolean" default="false">
  Enable the embedded Management Dashboard served at `/ui`. The dashboard provides browser-based administration for servers, tools, tool groups, clients, users, groups, and settings. It is disabled by default so the gateway binary stays minimal for pure CLI/API deployments.
</ParamField>
...
Start in enterprise mode with the Management Dashboard:
