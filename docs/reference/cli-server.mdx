...
<ParamField body="--sqlite-db-path" type="string">
  Path to the SQLite database file to use when PostgreSQL is not configured. Defaults to `./mcpjungle.db`. Overrides the `SQLITE_DB_PATH` environment variable.
</ParamField>
...
Start with a hidden SQLite file:
