...
If you want to hide the SQLite file or place it elsewhere, set a custom path:
...
Deleting the SQLite file removes registered servers, tool groups, users, and other persisted MCPJungle state stored in that database.
...
3. `--sqlite-db-path` — uses the configured SQLite file path
4. `SQLITE_DB_PATH` — uses the configured SQLite file path
5. SQLite fallback — creates `mcpjungle.db` in the current directory
