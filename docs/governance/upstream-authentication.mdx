...
Mcpjungle now supports OAuth flows for upstream HTTP MCP servers.
...
Current upstream OAuth support is intentionally narrow:

- it currently applies to HTTP-based upstream MCP servers
- the stored upstream token is gateway-scoped, not per downstream user
- token/client secrets are persisted in the Mcpjungle database and should be treated as sensitive operator data
