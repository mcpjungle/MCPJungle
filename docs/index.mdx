...
description: "MCPJungle is a self-hosted MCP gateway that aggregates many MCP servers behind one endpoint, with support for local use, team deployments, and enterprise controls."
---

MCPJungle is a self-hosted MCP gateway that gives your AI clients one stable endpoint for tools, prompts, and resources across all your MCP servers.

Instead of wiring Claude, Cursor, Copilot, and internal agents to each MCP server individually, you register those servers once in MCPJungle and expose them through a single `/mcp` endpoint.
...
<CardGroup cols={2}>
  ...
  <Card title="Core Concepts" icon="diagram-project" href="/core-concepts">
    Learn how the gateway, CLI, servers, tools, prompts, groups, and access model fit together.
  </Card>
