...
| Mcpjungle gateway | Per-client server allow-lists | Stable | Supported through MCP client creation in enterprise mode or dashboard. |
| Mcpjungle gateway | User accounts | Stable | Supported for human operators in enterprise mode. |
| Mcpjungle gateway | Per-user server allow-lists | Stable | Admins set explicit server access per user from the dashboard. |
| Mcpjungle gateway | User groups with inherited allow-lists | Stable | Users inherit the group's server allow-list; per-user overrides take precedence. |
| Mcpjungle gateway | Self-service MCP client tokens | Stable | Non-admin users create their own tokens via the dashboard Settings page. |
...
| Mcpjungle client | Management Dashboard (Web GUI) | Stable | Available via `mcpjungle start --ui`. Covers servers, tools, tool groups, clients, users, groups, and settings. |
