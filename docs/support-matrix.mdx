...
| Dynamic sync | Upstream `notifications/tools/list_changed` | Limited | Supported for `streamable_http` upstream servers only. Requires the upstream server to advertise `tools.listChanged`. Not currently supported for `sse` or `stdio`. |
